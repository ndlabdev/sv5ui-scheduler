...
- **Scheduler**: event details with `detail="popover"`, `detail="slideover"` (a panel inside the calendar whose body `eventPanel` can replace) or `detail={false}`; `eventDetail` adds content to both. Popover actions carry tooltips, and with `eventPanel` set the popover gains an Open details button.
...
- **Scheduler**: a layout for phones below `compactBreakpoint` (640px of scheduler width): a one row toolbar with a view menu and short titles, `compactDays` days in the week view, details in a slide-over, and popovers kept inside the screen.
...
- **Scheduler**: recurring events with `freq`, `interval`, `byDay`, `count`, `until` and `exDates`, expanded for the visible range only. Other RFC 5545 fields are accepted, reported once in development and ignored.
...
- **Scheduler**: snippets for `event`, `cell`, `header`, `eventDetail`, `eventPanel`, `createPanel`, `empty`, `toolbar` and `toolbarActions`; `toolbar={false}` hides the toolbar. A custom `event` keeps the click behaviour of the default chip: `onEventClick`, the configured `detail` and selection.
