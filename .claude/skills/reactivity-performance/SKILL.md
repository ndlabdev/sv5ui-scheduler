...
3. **An object handed to attachments must be stable; make its reactive fields getters.** `interactionContext` was a `$derived` object literal, so every focus change produced a new object, every plugin re-attached and a drag in progress was torn down by its own focus update. It is now one object created once in `components/Scheduler/interaction-state.svelte.ts`, with `get focus() { return this.focus }` style getters for the reactive parts. `src/tests/interactions.svelte.spec.ts` drives real gestures and would loop or drop the gesture if this regressed.
